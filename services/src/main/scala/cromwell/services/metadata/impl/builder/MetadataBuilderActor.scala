...
class MetadataBuilderActor(readMetadataWorkerMaker: () => Props)
  ...
  when(Idle) {
    ...
    case Event(action: SwitchToWaitMetadata, _) =>
      val readActor = context.actorOf(readMetadataWorkerMaker.apply())
      readActor ! action
      val req =  action.requester
      req ! ReadyToBuildResponse
  ...
  when(WaitingForMetadataService) {
    ...
    case Event(MetadataLookupResponseWithRequester(query, metadata, requester), IdleData) =>
      processMetadataResponseWithRequester(query, metadata, requester)
  ...
  def buildAndStop(query: MetadataQuery, eventsList: Seq[MetadataEvent], expandedValues: Map[String, JsValue], req: ActorRef, originalRequest: MetadataReadAction) = {
    val groupedEvents = groupEvents(eventsList)
    req ! BuiltMetadataResponse(originalRequest, processMetadataEvents(query, groupedEvents, expandedValues))
    allDone
  }

def buildAndStopWithRequester(query: MetadataQuery, eventsList: Seq[MetadataEvent], expandedValues: Map[String, JsValue], req: ActorRef) = {
    val groupedEvents = groupEvents(eventsList)
    req ! BuiltMetadataResponse(MetadataEmptyReadAction, processMetadataEvents(query, groupedEvents, expandedValues))
    allDone
  }

  def processMetadataResponseWithRequester(query: MetadataQuery, eventsList: Seq[MetadataEvent], req: ActorRef) = {
    buildAndStopWithRequester(query, eventsList, Map.empty, req)
  }

  def processMetadataResponse(query: MetadataQuery, eventsList: Seq[MetadataEvent], req: ActorRef, originalRequest: MetadataReadAction) = {
    if (query.expandSubWorkflows) {
      ...
      if (subWorkflowIds.isEmpty) buildAndStop(query, eventsList, Map.empty, req, originalRequest)
      else {
        ...
        goto(WaitingForSubWorkflows) using HasReceivedEventsData(req, originalRequest, query, eventsList, Map.empty, subWorkflowIds.size)
      }
    } else {
      buildAndStop(query, eventsList, Map.empty, req, originalRequest)
    }
  }
}
